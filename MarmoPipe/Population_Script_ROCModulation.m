...
UsuRoc = std(PopURoc)/sqrt(N);
 ...
 h = plot(ScatURoc(:,1),ScatARoc(:,1),'k.'); hold on;
 set(h,'Markersize',20);
 for k = 1:size(ScatURoc,1)
     plot([ScatURoc(k,1),ScatURoc(k,1)],...
          [ScatARoc(k,2),ScatARoc(k,3)],'k-'); hold on;
     plot([ScatURoc(k,2),ScatURoc(k,3)],...
          [ScatARoc(k,1),ScatARoc(k,1)],'k-'); hold on;
 end
...
%                                'HorizontalAlignment','right');
 ...
 ax.FontSize = 16;
 V = axis;
 t = xlabel('Saccade Away AUC');
 t(1).FontSize = 18;
 t = ylabel('Saccade Towards AUC');
 t(1).FontSize = 18;
 t = title('Population AUC');
 t(1).FontSize = 22;
