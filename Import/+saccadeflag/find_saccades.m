function saccades = find_saccades(timestamps, xpos, ypos,varargin)
...
n = ceil(args.dt*fs) + ceil(args.isi*fs/2); % samples
...
% clean up idx overlap
n = ceil(args.isi*fs);
nsac = size(idx,1);
isac = 1;
while isac < nsac
    ii = (idx(isac+1,1) - idx(isac,3)) < n;
    while ii
        idx(isac,3) = idx(isac+1,3);
        idx(isac+1,:) = [];
        nsac = nsac - 1;
        if isac + 1 > nsac
            break
