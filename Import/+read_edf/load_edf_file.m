...
load(filename);
if ~isstruct(edf)
    fprintf(fid, 'Failed to load EDF file %s\n',filename);
    fprintf(fid, 'Returning very unsatisfied ....\n');
    return;
end
...
if (MEDFILT > 0)
    fprintf(fid, 'Median Filtering Eye Data (+/- %d samples)\n',MEDFILT);
    for k = (MEDFILT+1):(size(dat,1)-MEDFILT-1)
        ...
        %**********
        if (mod(k,50000) == 0)
            fprintf(fid, 'Med filt EDF rawdata, count %d\n',k);
        end
        %*********
    end
end
%***** perform Gaussian smoothing with 5ms sigma window ************
fprintf(fid, 'Smoothing Gaussian Kernel (sig = %5.1f ms)\n',GSIG);
...
vpx.smo = mdat;
vpx.dat = mdat;
fprintf(fid, 'Completed vpx smoothing and upsampling\n');
