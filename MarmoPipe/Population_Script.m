...
if (athome == 1)
   ...
   %INFO_DATA_DIR = 'C:\Users\mitchelllab\Dropbox\FovealTrans\DataAnalysis\InfoSU';
   INFO_DATA_DIR = 'C:\Users\Shanna\Dropbox\FovealTrans\DataAnalysis\Info2';
end
...
for k = 3:size(xdir,1)
       ...
       NPopAuuStim = [NPopAuuStim ; (Info.AuuStim * (100/MaxStim))];
       NPopUuuStim = [NPopUuuStim ; (Info.UuuStim * (100/MaxStim))];
       NPopAuuPre =  [NPopAuuPre  ; (Info.AuuPre * (100/MaxPre))];
       NPopUuuPre =  [NPopUuuPre  ; (Info.UuuPre * (100/MaxPre))];
       %*******
       BefStim = Info.BefStim;
       AftStim = Info.AftStim;
       BefPre = Info.BefSac;
       AftPre = Info.AftSac;
       StimWin = Info.StimWin;
       PreWin = Info.PreWin;
       %*********
       N = N + 1;
       clear Info;
...
UsuPre = std(NPopUuuPre)/sqrt(N);
 ...
 set(h2,'Linewidth',2);
 h2b = plot(-BefStim:AftStim,AuuStim+(SM*AsuStim),'r-');hold on;
 h2b = plot(-BefStim:AftStim,AuuStim-(SM*AsuStim),'r-');hold on;
 %****** subset of trials where ori change DID occur
 h2 = plot(-BefStim:AftStim,UuuStim,'b-');hold on;
 set(h2,'Linewidth',2);
 h2b = plot(-BefStim:AftStim,UuuStim+(SM*UsuStim),'b-');hold on;
 h2b = plot(-BefStim:AftStim,UuuStim-(SM*UsuStim),'b-');hold on;
 %************
 axis tight;
 V = axis;
 ...
 xlabel('Time (ms)');
 ylabel('Norm. Rate (hz)');
 title('Stim-Locked Towards vs Away');

%******* plot stim locked PSTH
 subplot('position',[0.55 0.2 0.35 0.6]);
 %****** subset of trials where no ori change occured
 h2 = plot(-BefPre:AftPre,AuuPre,'r-');hold on;
 set(h2,'Linewidth',2);
 h2b = plot(-BefPre:AftPre,AuuPre+(SM*AsuPre),'r-');hold on;
 h2b = plot(-BefPre:AftPre,AuuPre-(SM*AsuPre),'r-');hold on;
 %****** subset of trials where ori change DID occur
 h2 = plot(-BefPre:AftPre,UuuPre,'b-');hold on;
 set(h2,'Linewidth',2);
 h2b = plot(-BefPre:AftPre,UuuPre+(SM*UsuPre),'b-');hold on;
 h2b = plot(-BefPre:AftPre,UuuPre-(SM*UsuPre),'b-');hold on;
 %************
 axis tight;
 V = axis;
 ...
 xlabel('Time (ms)');
 ylabel('Norm. Rate (hz)');
 title('Sac-Locked Towards vs Away');
...
%   subplot('position',[0.225 0.60 0.125 0.175])
%   %****** subset of trials where no ori change occured
%   xx = 1:(NPreBin-1);
...
%   subplot('position',[0.05 0.35 0.125 0.175]);
%   %****** subset of trials where no ori change occured
...
%   subplot('position',[0.225 0.35 0.125 0.175])
%   %****** subset of trials where no ori change occured
...
%   
%   %********* Stim - attended, pref vs non-pref
%   subplot('position',[0.05 0.075 0.125 0.175])
%   %****** subset of trials where no ori change occured
...
%   
%   %********* PreSac - attended, pref vs non-pref
%   subplot('position',[0.225 0.075 0.125 0.175])
%   %****** subset of trials where no ori change occured
