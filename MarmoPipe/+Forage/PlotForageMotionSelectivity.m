...
%   output: plot of spike raster sorted by motion direction
  ...
  AftStim = 500;  % so will convert to ms at some point
  ...
  %****** compute average over all speeds
  xx = [];
  uu = [];
  su1 = [];
  su2 = [];
  for k = 1:MoOri
     kks = ((1:MoSpd)-1)*MoOri + k;
     zz = find( ismember(PostDir,kks) == 1);
     xx = [xx ((k-1) * 360 / MoOri)];
     [a,b] = BootStrap( PostSpk(zz), BootN);
     uu = [uu a ];
     su1 = [su1 b(1)];
  ...
  for k = 1:MoNum
     mospd = 1 + (floor((k-1)/MoOri)); 
     [uu,su] = compute_psth(Rast{k},Smoothing);
     h2 = plot(-BefStim:AftStim,uu,[colo(mospd),'-']);hold on;
  end
  axis tight;
  V = axis;
  ...
  for k = 1:MoNum
      mospd = 1 + (floor((k-1)/MoOri)); 
      hh = plot(RastList{k}(:,1),tcount+RastList{k}(:,2),[colo(mospd),'.']);
