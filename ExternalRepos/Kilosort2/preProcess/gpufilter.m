...
if isfield(ops,'fslow')&&ops.fslow<ops.fs/2
    [b1, a1] = butter(4, [ops.fshigh/ops.fs,ops.fslow/ops.fs]*2, 'bandpass'); % butterworth filter with only 3 nodes (otherwise it's unstable for float32)
else
    [b1, a1] = butter(5, ops.fshigh/ops.fs*2, 'high'); % the default is to only do high-pass filtering at 150Hz
end
...
% if getOr(ops, 'gfilter', 0) > 0    
%     dataRAW = imgaussfilt(dataRAW, ops.gfilter); 
% end
...
if getOr(ops, 'gfilter', 0) > 0    
    datr = imgaussfilt(datr, ops.gfilter); 
end
