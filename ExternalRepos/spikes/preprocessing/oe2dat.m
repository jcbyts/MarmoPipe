...
oe_dir = dir(fullfile(oe_path, '*CH*.continuous'));

chnum = arrayfun(@(x) str2double(cell2mat(regexp(x.name,'(?<=CH[^0-9]*)[0-9]*\.?[0-9]+', 'match'))), oe_dir);
[~, ind] = sort(chnum);
oe_ch_filenames = arrayfun(@(x) fullfile(oe_path, x.name), oe_dir(ind), 'uni', 0);
