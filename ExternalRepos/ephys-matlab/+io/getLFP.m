...
Nsample = 2^nextpow2(nTotSamp); %2^nextpow2(min(nTotSamp, info.sampleRate*CHUNK_SIZE)); % ~500s chunks
...
if block == nBlocks
    Nsample = 2^nextpow2(size(dataRAW,1)); %2^nextpow2(min(size(dataRAW,1), info.sampleRate*CHUNK_SIZE)); % ~500s chunks
