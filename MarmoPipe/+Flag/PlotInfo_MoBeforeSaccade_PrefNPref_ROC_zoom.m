...
%               max(UuuStimPref+(2*UsuStimPref)),max(UuuPreNPref+(2*UsuPrePref))]);
  ...
  %****** subset of trials where no ori change occured
  h2 = plot(-BefSac:AftSac,AuuPrePref,'r-');hold on;
  set(h2,'Color',[1.0000 0.6000 0.7843])
  set(h2,'Linewidth',2);
  h2b = plot(-BefSac:AftSac,AuuPrePref+(2*AsuPrePref),'r-');hold on;
  set(h2b,'Color',[1.0000 0.6000 0.7843])
  h2b = plot(-BefSac:AftSac,AuuPrePref-(2*AsuPrePref),'r-');hold on;
  set(h2b,'Color',[1.0000 0.6000 0.7843])
  %****** subset of trials where ori change DID occur
  h2 = plot(-BefSac:AftSac,AuuPreNPref,'r-');hold on;
  set(h2,'Linewidth',2);
  h2b = plot(-BefSac:AftSac,AuuPreNPref+(2*AsuPreNPref),'r-');hold on;
  h2b = plot(-BefSac:AftSac,AuuPreNPref-(2*AsuPreNPref),'r-');hold on;
  %************
  axis tight;
  V = axis;
  %axis([-BefSac AftSac 0 Vmax]);
  %axis([-200 AftSac 0 300]);
  axis([-100 0 0 Vmax]);
  plot([0,0],[0,Vmax],'k-');
  ...
  h2 = plot(-BefSac:AftSac,UuuPrePref,'b-');hold on;
  set(h2,'Color',[ 0.5843 0.8157 0.9882])
  set(h2,'Linewidth',2);
  h2b = plot(-BefSac:AftSac,UuuPrePref+(2*UsuPrePref),'b-');hold on;
