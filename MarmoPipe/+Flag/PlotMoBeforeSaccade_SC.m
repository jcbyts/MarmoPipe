...
%     Info - a struct with necessary results for plots and for pop analyses
  ...
  AftStim = 200;  % so will convert to ms at some point
  %****** selects windows for analysis (non-overlapping now)
  PreWin = [-100,25];  % for plotting purposes
  StimWin = [50,175]; % for plotting
  %*********
  PreWinWid = 40;
  ...
  OriVals = Orivals;
  [Afitstim,Atunestim,Arawstim] = FitWithVonMises(OriVals,JackN,TrA,zPreOri,StimSpk{NStimBin});
  [Ufitstim,Utunestim,Urawstim] = FitWithVonMises(OriVals,JackN,TrU,zPreOri,StimSpk{NStimBin});
  TrT = [TrA ; TrU]; % everything
  [Tfitstim,Ttunestim,Trawstim] = FitWithVonMises(OriVals,JackN,TrT,zPreOri,StimSpk{NStimBin});
  %*******
  [Afitpre,Atunepre,Arawpre] = FitWithVonMises(OriVals,JackN,TrA,zPreOri,PreSpk{NPreBin});
  [Ufitpre,Utunepre,Urawpre] = FitWithVonMises(OriVals,JackN,TrU,zPreOri,PreSpk{NPreBin});
  %**********
  ...
  Flag.PlotInfo_MoBeforeSaccade_SC(Info);  % plot out everything
...
function [Afit,Atune,Araw] = FitWithVonMises(OriVals,JackN,TrA,zPreOri,StimSpk)
