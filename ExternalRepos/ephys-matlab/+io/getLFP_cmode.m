function [data, timestamps, info] = getLFP_cmode(ops, overwrite, plotIt)
...
% 2020.01.14    jfm     added common mode rejection before artifact removal
...
% --- Declare constants
LOW_CUTOFF  = 300;
ARTIFACT_SD = 0; %5; %if zero, do not apply, else reject anything outside
...
fprintf('1) Perform common mode rejection across channels \n')
fprintf('2) Correct phase distortion from OE headstage highpass at %dHz...\n', OE_HIGHPASS)
fprintf('3) Lowpass filter at %dHz...\n', LOW_CUTOFF)
fprintf('4) Downsample to %dHz...\n', NEW_FS)
if (ARTIFACT_SD)
   fprintf('4b..) Reject artifacts bigger than %d SD...\n', ARTIFACT_SD)
end
...
disp(' ');
disp('Common mode rejection ...');
dataMEAN = mean(dataRAW,2);
for ch = 1:Nchan
   dataRAW(:,ch) = dataRAW(:,ch) - dataMEAN; 
end

disp('Phase distortion ...');
if block == nBlocks
...
% return the full data struct, not simply the final fragment being appended
if exist(flfp, 'file')
    fid = fopen(flfp, 'r');
