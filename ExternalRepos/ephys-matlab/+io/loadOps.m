...
if isa(fpath, 'table')
    ...
    fpath = fullfile(SERVER_DATA_DIR, fpath.Directory{1});    
    % fpath = fullfile(getpref('EPHYS', 'SERVER_DATA'), fpath.Directory{1});
end

warning('off') % suppress warning while loading
processedDir = dir(fullfile(fpath, '*processed*'));
shanksDir = dir(fullfile(fpath, '_shank*'));
shankDirs = [processedDir; shanksDir];
