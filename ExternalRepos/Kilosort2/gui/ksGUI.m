...
%             obj.updateProbeView('new');
        ...
        function runSaveToPhy(obj)            
            ...
            rez.cProj = [];
            rez.cProjPC = [];
            rez.ops.gui = []; % remove GUI because it recursively points ot itself
            ...
            if obj.P.dataGood && obj.P.probeGood
                ...
                if all(samps>0 & samps<obj.P.datSize(2))
                    ...
                    for q = 1:numel(chList)
                        if ~isempty(intersect(chList(q), cmconn))%obj.P.chanMap.connected(obj.P.chanMap.chanMap(chList(q)))
...
%                 load('defaultOps.mat', 'ops');
            if exist('configMitchellLab.m', 'file')
                configMitchellLab;  
                ops.trange      = [0 Inf];
            else
                ops = [];
